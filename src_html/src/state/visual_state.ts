...
export const [sidebar, setSidebar]: Signal<SidebarState> = createSignal(initSidebarSignal());
...
function initThemeSignal(): Theme{
    let default_theme = window.matchMedia("(prefers-color-scheme: dark)")
    .matches
    ? "navy"
    ...
    if (themeUnvalidated === null || themeUnvalidated === undefined || !["light","rust","coal","navy","ayu"].includes(themeUnvalidated)) {
